...
{	
	...
	m_CurrentReportFolder += m_CrashDateTime.toString("dd.MM.yyyy hh.mm.ss");
...
bool ErrorReport::GenerateArchive()
{
...
#ifndef USE_MINIZIP
	HZIP hz = CreateZip(fullPath.toAscii(), 0);
	...
	{
		zResult = ZipAdd(hz, fn.toAscii(),  fn.toAscii());
		if(zResult != ZR_OK)
			break;
	}
	...
	if(err != ZIP_OK)
	{
		zipClose(zf, "Generated by ArxLibertatis crash reporter");
		return false;
	}

	err = zipClose(zf, "Generated by ArxLibertatis crash reporter");
	return err == ZIP_OK;
#endif
}

bool ErrorReport::Send()
{
	std::string fileName = m_SharedMemoryName + ".zip";
	QString fullPath = GetFilePath(fileName);
	...
	smptClient.SetSubject("Arx Fatalis Crash Report");
	smptClient.AddMsgLine("Hello");
	smptClient.AddAttachment(fullPath.toAscii());
	smptClient.SetLogin("arxlibertatis.crashes@gmail.com");
	smptClient.SetPassword("yu8pnioo");
	smptClient.SetSMTPServer("smtp.gmail.com", 465, true);
	smptClient.SetSecurityType(USE_SSL);
	smptClient.Send();
