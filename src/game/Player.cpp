...
void PlayerMovementIterate(float DeltaTime) {
	...
	if(USE_PLAYERCOLLISIONS) {
		...
		float jump_mul = 1.f;
		ArxDuration diff = ArxDurationMsf(float(toMs(arxtime.now() - LAST_JUMP_ENDTIME)) * (1.0f / GLOBAL_SLOWDOWN));
		...
		if(glm::abs(player.physics.velocity.x) < 0.001f
		   ...
		   && player.jumpphase == NotJumping
			...
			if(player.jumpphase == JumpAscending) {
				...
				const float elapsed = float(toMs(now - player.jumpstarttime)) * (1.0f / GLOBAL_SLOWDOWN);
