...
struct EERIE_CAMERA {
	...
	void setTargetCamera(float x, float y, float z)
	{
		...
		angle.setYaw((degrees(getAngle(orgTrans.pos.y, orgTrans.pos.z, y, orgTrans.pos.z + dist(Vec2f(x, z), Vec2f(orgTrans.pos.x, orgTrans.pos.z)))))); //alpha entre orgn et dest;
