...
{
	long iCancel = ARX_SPELLS_GetInstanceForThisCaster(SPELL_RUNE_OF_GUARDING, spells[i].m_caster);
	if(iCancel > -1) {
	...
	spells[i].m_tolive = (duration > -1) ? duration : 99999999;
	
	CRuneOfGuarding * effect = new CRuneOfGuarding();
	effect->spellinstance = i;
	effect->Create(entities[spells[i].m_caster]->pos, 0);
...
void RuneOfGuardingSpell::Update(size_t i, float timeDelta)
{
	if(spells[i].m_pSpellFx) {
		...
		CRuneOfGuarding * pCRG=(CRuneOfGuarding *)spells[i].m_pSpellFx;

		if (pCRG)
		{
			EERIE_SPHERE sphere;
			sphere.origin = pCRG->eSrc;
			sphere.radius=std::max(spells[i].m_caster_level*15.f,50.f);

			if (CheckAnythingInSphere(&sphere,spells[i].m_caster,CAS_NO_SAME_GROUP | CAS_NO_BACKGROUND_COL | CAS_NO_ITEM_COL| CAS_NO_FIX_COL | CAS_NO_DEAD_COL))
			{
				ARX_BOOMS_Add(pCRG->eSrc);
				LaunchFireballBoom(&pCRG->eSrc,(float)spells[i].m_caster_level);
				DoSphericDamage(&pCRG->eSrc,4.f*spells[i].m_caster_level,30.f*spells[i].m_caster_level,DAMAGE_AREA,DAMAGE_TYPE_FIRE | DAMAGE_TYPE_MAGICAL,spells[i].m_caster);
				spells[i].m_tolive=0;
...
{
	long iCancel = ARX_SPELLS_GetInstanceForThisCaster(SPELL_LEVITATE, spells[i].m_caster);
	if(iCancel > -1) {
		spells[iCancel].m_tolive = 0;
	}
	...
	ARX_SOUND_PlaySFX(SND_SPELL_LEVITATE_START, &entities[spells[i].m_target]->pos);
	spells[i].m_exist = true;
	spells[i].m_tolive = (duration > -1) ? duration : 2000000000;
	...
	if(spells[i].m_target == 0) {
		...
		spells[i].m_tolive = 200000000;
		player.levitate = true;
	} else {
		target = entities[spells[i].m_target]->pos;
	}
	
	effect->Create(16, 50.f, 100.f, 80.f, &target, spells[i].m_tolive);
	spells[i].m_pSpellFx = effect;
	spells[i].m_tolive = effect->GetDuration();
	
	spells[i].m_snd_loop = ARX_SOUND_PlaySFX(SND_SPELL_LEVITATE_LOOP,
	                                       &entities[spells[i].m_target]->pos, 0.7f,
...
void LevitateSpell::End(size_t i)
{
	...
	ARX_SOUND_PlaySFX(SND_SPELL_LEVITATE_END, &entities[spells[i].m_target]->pos);
...
{
	CLevitate *pLevitate=(CLevitate *)spells[i].m_pSpellFx;
	Vec3f target;
	...
	if(spells[i].m_target == 0) {
		...
		target.x = entities[spells[i].m_caster]->pos.x;
		target.y = entities[spells[i].m_caster]->pos.y;
		target.z = entities[spells[i].m_caster]->pos.z;
	}
...
void CurePoisonSpell::Launch(long i)
{
	...
	float cure = spells[i].m_caster_level * 10;
	if(spells[i].m_target == 0) {
		player.poison -= std::min(player.poison, cure);
		ARX_SOUND_PlaySFX(SND_SPELL_CURE_POISON);
	} else if (ValidIONum(spells[i].m_target)) {
		Entity * io = entities[spells[i].m_target];
...
{
	long iCancel = ARX_SPELLS_GetInstanceForThisCaster(SPELL_REPEL_UNDEAD, spells[i].m_caster);
	if(iCancel > -1) {
		spells[iCancel].m_tolive = 0;
	}
	...
	ARX_SOUND_PlaySFX(SND_SPELL_REPEL_UNDEAD, &entities[spells[i].m_target]->pos);
	if(spells[i].m_target == 0) {
		spells[i].m_snd_loop = ARX_SOUND_PlaySFX(SND_SPELL_REPEL_UNDEAD_LOOP,
		                                       &entities[spells[i].m_target]->pos, 1.f,
		                                       ARX_SOUND_PLAY_LOOPED);
	}
	
	spells[i].m_exist = true;
	spells[i].m_tolive = (duration > -1) ? duration : 20000000;
	spells[i].m_bDuration = true;
	spells[i].m_fManaCostPerSecond = 1.f;
...
void PoisonProjectileSpell::Launch(long i)
{
	ARX_SOUND_PlaySFX(SND_SPELL_POISON_PROJECTILE_LAUNCH,
	                  &spells[i].m_caster_pos);
	
	spells[i].m_exist = true;
	spells[i].m_tolive = 900000000; // TODO probably never read
	
	long level = std::max(long(spells[i].m_caster_level), 1l);
	...
	if(spells[i].m_caster == 0) {
		...
		ang = entities[spells[i].m_caster]->angle.getPitch();
	}
