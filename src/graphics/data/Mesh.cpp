...
Vec3f EE_RT(const Vec3f & in) {
	return Vec3f(ACTIVECAM->m_worldToView * Vec4f(in, 1.0f));
}
...
static Vec4f viewToClipSpace(const Vec3f & in) {
	...
	Vec4f out;
	out.x = in.x * ACTIVECAM->m_viewToScreen[0][0] + z * ACTIVECAM->m_viewToScreen[2][0];
	out.y = in.y * ACTIVECAM->m_viewToScreen[1][1] + z * ACTIVECAM->m_viewToScreen[2][1];
	out.z = in.z * ACTIVECAM->m_viewToScreen[2][2] + ACTIVECAM->m_viewToScreen[3][2];
	out.w = z;
	return out;
}
