...
void LaunchPoisonExplosion(Vec3f * aePos) {
	...
	cp.m_endColor[3] = 0;
	cp.m_endColorRandom[0] = 0;
	cp.m_endColorRandom[1] = 25;
	cp.m_endColorRandom[2] = 0;
	cp.m_endColorRandom[3] = 20;
	cp.m_endLock = false;
...
void CPoisonProjectile::Create(Vec3f _eSrc, float _fBeta)
{
	...
	cp.m_startSizeRandom = 3;
	cp.m_startColor[0] = 0;
	cp.m_startColor[1] = 50;
	cp.m_startColor[2] = 0;
	cp.m_startColor[3] = 40; 
	cp.m_startColorRandom[0] = 0;
...
void CPoisonProjectile::Update(unsigned long _ulTime)
{
	...
	if(ulCurrentTime < 750) {
		...
		if(!bOk) {
			...
			cp.m_startColor[0] = 0;
			cp.m_startColor[1] = 39;
			cp.m_startColor[2] = 0;
			cp.m_startColor[3] = 100;
			cp.m_startColorRandom[0] = 50;
