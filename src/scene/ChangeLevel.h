...
struct SavedGlobalMods {
	...
	inline SavedGlobalMods & operator=(const GLOBAL_MODS & b) {
		...
		zclip = b.zclip;
		assert(sizeof(b.ambiance) == sizeof(ambiance));
		std::copy(b.ambiance, b.ambiance + sizeof(ambiance), ambiance);
	...
	inline operator GLOBAL_MODS() {
		...
		a.zclip = zclip;
		assert(sizeof(a.ambiance) == sizeof(ambiance));
		std::copy(ambiance, ambiance + sizeof(ambiance), a.ambiance);
...
struct SavedMapMakerData {
	...
	inline SavedMapMakerData(const MAPMARKER_DATA & b) {
		...
		y = b.y;
		lvl = b.lvl;
		assert(sizeof(string) == sizeof(b.string));
		std::copy(b.string, b.string + sizeof(string), string);
	}
	...
	inline operator MAPMARKER_DATA() {
		...
		a.y = y;
		a.lvl = lvl;
		assert(sizeof(a.string) == sizeof(string));
		std::copy(string, string + sizeof(string), a.string);
	}
...
struct SavedMiniMap {
	...
	inline operator MINI_MAP_DATA() {
		...
		std::copy(revealed, revealed + (SavedMiniMap::MAX_X * SavedMiniMap::MAX_Z), a.revealed);
		return a;
	}
	...
	inline SavedMiniMap & operator=(const MINI_MAP_DATA & b) {
		...
		assert(SavedMiniMap::MAX_X == MINIMAP_MAX_X);
		assert(SavedMiniMap::MAX_Z == MINIMAP_MAX_Z);
		std::copy(b.revealed, b.revealed + (SavedMiniMap::MAX_X * SavedMiniMap::MAX_Z), revealed);
		return *this;
	}
...
struct SavedSpellcastData {
	...
	inline operator IO_SPELLCAST_DATA() {
		...
		assert(sizeof(a.symb) == sizeof(symb));
		std::copy(symb, symb + sizeof(symb), a.symb);
	...
	inline SavedSpellcastData & operator=(const IO_SPELLCAST_DATA & b) {
		...
		assert(sizeof(b.symb) == sizeof(symb));
		std::copy(b.symb, b.symb + sizeof(symb), symb);
...
struct SavedTweakerInfo {
	...
	inline operator IO_TWEAKER_INFO() {
		IO_TWEAKER_INFO a;
		assert(sizeof(a.filename) == sizeof(filename));
		std::copy(filename, filename + sizeof(filename), a.filename);
		assert(sizeof(a.skintochange) == sizeof(skintochange));
		std::copy(skintochange, skintochange + sizeof(skintochange), a.skintochange);
		assert(sizeof(a.skinchangeto) == sizeof(skinchangeto));
		std::copy(skinchangeto, skinchangeto + sizeof(skinchangeto), a.skinchangeto);
		return a;
	}
	...
	inline SavedTweakerInfo(const IO_TWEAKER_INFO & b) {
		assert(sizeof(filename) == sizeof(b.filename));
		std::copy(b.filename, b.filename + sizeof(filename), filename);
		assert(sizeof(skintochange) == sizeof(b.skintochange));
		std::copy(b.skintochange, b.skintochange + sizeof(skintochange), skintochange);
		assert(sizeof(skinchangeto) == sizeof(b.skinchangeto));
		std::copy(b.skinchangeto, b.skinchangeto + sizeof(skinchangeto), skinchangeto);
	}
...
struct SavedGroupData {
	...
	inline operator IO_GROUP_DATA() {
		IO_GROUP_DATA a;
		assert(sizeof(a.name) == sizeof(name));
		std::copy(name, name + sizeof(name), a.name);
		return a;
	}
	...
	inline SavedGroupData(const IO_GROUP_DATA & b) {
		assert(sizeof(name) == sizeof(b.name));
		std::copy(b.name, b.name + sizeof(name), name);
	}
...
struct SavedTweakInfo {
	...
	inline operator TWEAK_INFO() {
		TWEAK_INFO a;
		a.type = type;
		assert(sizeof(a.param1) == sizeof(param1));
		std::copy(param1, param1 + sizeof(param1), a.param1);
		assert(sizeof(a.param2) == sizeof(param2));
		std::copy(param2, param2 + sizeof(param2), a.param2);
		return a;
	}
	...
	inline SavedTweakInfo(const TWEAK_INFO & b) {
		type = b.type;
		assert(sizeof(param1) == sizeof(b.param1));
		std::copy(b.param1, b.param1 + sizeof(param1), param1);
		assert(sizeof(param2) == sizeof(b.param2));
		std::copy(b.param2, b.param2 + sizeof(param2), param2);
	}
